...
namespace TestAppWithNLog.Commands
{
    ...
    public class AlgoCommand
    {
        ...
        public void Execute()
        {
            ...
            logger.Info("Algorithm with Logging calculated in {Seconds}, Answer: {Answer}",     algoWithLogs.neededTime,    algoWithLogs.result);
            logger.Info("Algorithm without Logging calculated in {Seconds}, Answer: {Answer}",  algoWithoutLogs.neededTime, algoWithoutLogs.result);
        }
        ...
        public static S_AlgorithmResults RunAlgorithm(bool loggingEnabled, double baseValue, double wantedPrecision)
        {
            ...
            while ((high - low) > wantedPrecision)
            {
                ...
                if (loggingEnabled)
                {
                    logger.Trace("Algorithm running, currently on iteration: {Iteration}", counter);
                }
